...
namespace LeetCode.June
{
	public class CheapestFlightsWithinKStops
	{
        ...
        {
            map = new Dictionary<int, Dictionary<int, int>>(n);
            foreach (var flight in flights)
            {
                var key = flight[0];
                if (map.ContainsKey(key))
                {
                    map[key].Add(flight[1], flight[2]);
                }
                else
                {
                    var destins = new Dictionary<int, int>
                    {
                        { flight[1], flight[2] }
                    };
                    map.Add(key, destins);
        ...
        public void Test()
		{
			...
			var result = FindCheapestPrice(3, flights, 0, 2, 1);
