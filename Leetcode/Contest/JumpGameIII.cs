...
namespace Contest
{
    public class JumpGameIII
    {
        ...
        public static bool Jump(int[] arr, int start)
        {
            count++;
            ...
            var index1 = start + arr[start];
            var index2 = start - arr[start];
            if (index1 < arr.Length)
            {
                return Jump(arr, index1);
            }

            if (index2 >= 0)
            {
                return Jump(arr, index2);
            }
        ...
        {
            count = 0;
            var isReachable = Jump( arr, start);
